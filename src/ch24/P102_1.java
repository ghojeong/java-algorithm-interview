    ...
    public int dfs(int playerRow, int playerCol, int opponentRow, int opponentCol) {
        ...
        for (int[] direction : directions) {
            ...
            if (playerNextRow < 0
                    ...
                    || playerNextCol < 0
                    || playerNextCol >= M
                    || board[playerNextRow][playerNextCol] == 0) continue;
            ...
            if (result % 2 == 0 && moveCount % 2 == 1) result = moveCount;
            // 지는 줄 알았는데 지는 경우: 이동 횟수 최대화
            else if (result % 2 == 0 && moveCount % 2 == 0) result = Math.max(result, moveCount);
            // 이기는 줄 알았는데 이기는 경우: 이동 횟수 최소화
            else if (result % 2 == 1 && moveCount % 2 == 1) result = Math.min(result, moveCount);
